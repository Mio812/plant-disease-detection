...
| H17 | Classification training on PlantVillage actively destroys severity information, because colour and texture augmentation exists to make the model invariant to exactly what severity is made of. Training both objectives together should recover it, possibly at a small cost in classification accuracy. | **confirmed, with a ceiling.** Joint training (E26) makes the head reproduce the official mask near-perfectly (within-class rho 0.957, reproduced, non-degenerate) at no classification cost (99.44%). Against *human* grades (n=20 test subset) it reaches rho 0.47, beating Otsu 0.43 and the mask 0.41 — best estimator and a model output, but lesion-area severity plateaus below human reliability |
...
| E14 | Ordinal grade vs manual annotation (ρ, MAE, κ) | H6 | 150 leaves, **three annotators**; bands fixed to the rubric before grading | **done.** Mean pairwise κ 0.63 (ceiling; A-B 0.72, A-C 0.54, B-C 0.64). Model vs median consensus: ρ 0.46 (p<1e-8), κ 0.30 rubric → **0.47 recalibrated (5-fold CV)** — reaches ~74% of the human ceiling, a moderate proxy |
...
| E26 | Joint training: backbone updated by classification **and** severity loss together | H17 | three measured controls on one test split — fine-tuned frozen features 0.41, ImageNet frozen features 0.61, Otsu 0.78. Must clear **0.78** within-class to make severity worth taking from the network | **done.** Clears the vs-official bar at 0.957 (a mask emulator), classification 99.44%. Human cross-check (n=20): head 0.47 > Otsu 0.43 > mask 0.41 — best, and a model output, but all lesion-area estimators plateau near human rho 0.47 |
...
| Robustness to lighting, growth stage and symptom variation | E21 stratification; E8, E10 | E21: field accuracy is flat across luminance/contrast/sharpness bins (gaps < 1.6 pt on luminance/contrast), so lighting variation is *not* what breaks it — the domain gap is. Reported as evaluated under natural variation. Growth stage has no labels, recorded as a limitation |
