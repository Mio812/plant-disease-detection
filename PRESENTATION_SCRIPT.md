...
> photographs taken in actual fields. Accuracy fell from 99.53% to **16.1%**.
>
> That 83-point gap is our project. The rest of this presentation is why it
> happens, and what actually closes it.

*讲这段时放慢，"99.53 → 16.1" 是全场最重要的一组数字，说完停一秒。*

### Slide 3 · Problem statement (55s)

> We turned that into three research questions.
>
> **RQ1** is the brief's first task — can a CNN identify the crop and its
> condition. 38 classes across 14 crops.
>
> **RQ2** is the question the brief implies but does not state: does laboratory
> accuracy transfer to the field?
>
> **RQ3** is the brief's second task — can severity be estimated from image
> features, and can the estimate be trusted?
>
> And three safeguards, because we learned early that this dataset punishes
> carelessness. We split by **physical leaf**, not by image. We audit for
> background shortcuts. And PlantDoc is never trained on in any zero-shot
> experiment.
...
## Before you present — deck fixes needed
...
3. **Slide 2 speaker note (Chinese)** says the model memorises 灰度背景. That is
   not what we found: the shortcut is the **background/backdrop**, measured by
   the border-pixel probe at 33.7%. The grayscale arm is a separate experiment
   (97.76% trained on grayscale). Fix the note so nobody says it out loud.
4. **Slide 5** reads "4 samples each: colour · grayscale · segmented" — three
   variants under the label "4 samples". Reword to "3 variants per leaf".
5. **Slide 10** has no slide-number box; every other slide has one.
